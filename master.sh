...
sudo mysql -uroot -e "CREATE DATABASE Onifade;"
sudo mysql -uroot -e "CREATE USER 'julius'@'localhost' IDENTIFIED BY 'drixt';"
sudo mysql -uroot -e "GRANT ALL PRIVILEGES ON Onifade.* TO 'julius'@'localhost';"
cd /var/www/laravel
...
sudo sed -i '25 s/=laravel/=Onifade/' /var/www/laravel/.env
sudo sed -i '26 s/=root/=julius/' /var/www/laravel/.env
sudo sed -i '27 s/=/=drixt/' /var/www/laravel/.env
